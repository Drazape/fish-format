...
#### Position
Variants can only be used beside the used color. If it is used elsewhere, then it will be treated as part of the string.  
!!! tip "Single Variant syntactic advantages"
    Because there is currently only one variant, there are several advantages we can make use of to make the syntax less error-prone.

    - If it is used once, then you don't need to use `--` to use it in the string again, and all flags and `--` be interpreted as part of the string
